...
class ConnectFour:
    ...
    def win_check(self):
        red_win = self.score_red >= self.score_win
        blue_win = self.score_blue >= self.score_win
        full_board = all ([self.board.col_height(col) == self.board.height for col in range(self.board.width)])
